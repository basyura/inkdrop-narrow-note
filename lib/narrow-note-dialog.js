...
const NarrowNoteDialog = (_) => {
  ...
  const buildNotes = async (mode) => {
    ...
    // ダイアログの初期表示用に、更新日時が新しい順で取得したノート
    let initialNotes = [];
    if (mode == "book") {
      initialNotes = (await db.notes.findInBook(currentBookId, params)).docs;
    } else {
      initialNotes = await inkdrop.localDB.notes.searchWithQuery(
        [{ type: "field", field: "pinned", id: ["true", "false"] }],
        { sort: [{ updatedAt: "desc" }], limit: count }
      );
    }

    setOptions(
      initialNotes.map(({ _id, title, bookId, updatedAt, status }) => {
