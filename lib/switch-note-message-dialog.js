...
export default class SwitchNoteMessageDialog extends React.Component {
  constructor(props) {
    super(props);
    this.state = { options: [], bookId: "", dictionaryHash: 0 };
    this.selectedNoteId = "";
  }
  ...
  buildDictionary() {
    ...
    if (dictionaryHash === this.state.dictionaryHash) {
      ...
      const copied = JSON.parse(JSON.stringify(this.state.copied));
      this.setState({ options: copied });
      return;
    }
  ...
  switchNote = (_, data) => {
    console.log("switch-note:switchNote : " + data.value.note);
    // if (this.selectedNoteId == data.value.note) {
    //   console.log("skip");
    //   return;
    // }
    // this.selectedNoteId = data.value.note;
    //if (this.state.bookId !== data.value.book) {
    //  console.log("dispatch note-lsit-show");
    ...
    //
    setTimeout(() => {
      // console.log("dispatch open-noet");
    ...
    return (
      ...
      >
        <Dropdown
          ...
          searchInput={<Dropdown.SearchInput className="ui input" onKeyDown={this.handleKeyDown} />}
